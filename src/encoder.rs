...
impl Ac3Encoder {
    fn emit_syncframe(&mut self) -> Result<()> {
        ...
        // Now compute crc1 over the first 5/8 of the syncframe (excluding
        // the 2-byte sync word) and patch it into bytes [2..4].
        ...
        // §7.10.1: crc1 is generated so the CRC calculation produces zero
        // at the 5/8 point. i.e. crc1 = CRC(bytes_after_crc1_through_5/8)
        // with the generator x^16 + x^15 + x^2 + 1. The crc1 field itself
        // is *included* in that calculation (as zero placeholders) and
        // its final value is chosen to zero out the register.
        // Practically: compute the CRC of the frame from byte 4
        // (post-sync + post-crc1-zero) to byte five_eighths_bytes, then
        // take the inverse so that CRC(data || crc1) == 0.
        let crc1_bytes = &frame[4..five_eighths_bytes];
        let crc1_val = ac3_crc_inverse(crc1_bytes);
        frame[2] = (crc1_val >> 8) as u8;
        frame[3] = (crc1_val & 0xFF) as u8;

        // crc2 covers the last 3/8 of the syncframe: bytes
        // five_eighths_bytes .. (frame_bytes - 2) with the 2-byte crc2
        // field itself included as zero.
        let crc2_bytes = &frame[five_eighths_bytes..(self.frame_bytes - 2)];
        let crc2_val = ac3_crc_inverse(crc2_bytes);
...
/// Compute the crc1 / crc2 value that, when appended to the covered
/// region (which itself ends with a 2-byte zeroed CRC placeholder),
/// makes the register collapse to zero at the end of the shift.
///
/// §7.10.1: "crc1 is generated by encoders such that the CRC calculation
/// will produce zero at the 5/8 point in the syncframe." So encoders
/// compute CRC over (data || 0x0000) and then invert (treating CRC as
/// polynomial over GF(2)) to solve for the needed placeholder value.
///
/// A compact trick: CRC(D || X) = CRC(D)*x^16 + X*P(x) where P is the
/// polynomial structure. The standard solve-for-X is:
///
///   - Let R = CRC(D) -- CRC of the data *without* the trailing 2
///     zero bytes that represent the embedded CRC field.
///   - Then the CRC field value to prepend is the residue that cancels
///     R when further shifted through the register. Equivalently, we
///     pre-feed R through 16 more zero bits and read off the result.
///
/// Implementation: we run `ac3_crc_update` over `data` *excluding* the
/// trailing 16-bit CRC field — but in our callers the CRC field is
/// *embedded* in the data slice as zero bytes. Passing the full slice
/// (with 2-byte zero tail) and inverting gives us the value X such
/// that CRC(data || X) == 0.
fn ac3_crc_inverse(data_with_zero_crc: &[u8]) -> u16 {
    // Running CRC of the whole region including the 2 zero CRC bytes.
    let r = ac3_crc_update(0, data_with_zero_crc);
    // The two zero bytes pull `r` through 16 more shifts. To cancel `r`,
    // we need to XOR it out of the register — which is exactly what
    // placing `r` into those two slots would do, because the update
    // applies to `crc ^ X << 0` at each byte. Concretely:
    //   new_r = CRC(data || 0x0000) -- what we just computed
    //   We want CRC(data || X) == 0. CRC is linear over XOR, so
    //   CRC(data || X) = CRC(data || 0x0000) XOR CRC(0...0 || X).
    //   CRC(N zero bytes || X) where N = len(data) is just shifting X
    //   through the register `N+2`*8 times. But because the CRC field
    //   sits at the END and the LFSR shifts new bits into the low bit,
    //   the straightforward property is that CRC(data || X) =
    //   CRC(data) shifted 16 bits plus X (mod poly). So X that zeros
    //   the result is CRC(data) shifted 16 with the register's natural
    //   feedback.
    //
    // Because we pass zeros for those two trailing bytes, the computed
    // `r` IS already the residue-after-16-more-shifts value; the CRC
    // field we want to emit equals `r` itself.
    r
}
