fn st(data:&[u8],fi:usize)->oxideav_ac3::audblk::Ac3State {
    ...
    loop { let si=oxideav_ac3::syncinfo::parse(&data[off..]).unwrap(); let len=si.frame_length as usize;
        if n==fi { let f=&data[off..off+len]; let b=oxideav_ac3::bsi::parse(&f[5..]).unwrap();
...
fn main() {
    ...
    let s2=st(&data,2); let s4=st(&data,4);
    let mut ed=0; for b in 0..217 { if s2.channels[0].exp[b]!=s4.channels[0].exp[b] { ed+=1; } }
    println!("ch0 exp diffs: {ed}");
    for b in 0..217 {
        if s2.channels[0].bap[b]!=s4.channels[0].bap[b] {
            println!("bin {b}: exp {} vs {} bap {} vs {}", s2.channels[0].exp[b], s4.channels[0].exp[b], s2.channels[0].bap[b], s4.channels[0].bap[b]);
        }
    }
}
