...
fn census(stem: &str) {
    ...
    let (mut off, mut frames, mut blocks, mut short, mut frames_with_short) =
        (0, 0u32, 0u32, 0u32, 0u32);
    while off + 5 <= data.len() {
        let Ok(si) = syncinfo::parse(&data[off..]) else {
            break;
        };
...
fn per_frame(stem: &str, nframes: usize) {
    ...
    for f in 0..nframes {
        ...
        let short: u32 = blks
            .iter()
            .map(|sb| sb.blksw[..nf].iter().filter(|&&x| x).count() as u32)
            .sum();
        if short > 0 {
            print!("{f}({short}) ");
        }
