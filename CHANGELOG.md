...
- eac3: **opt-in JOC/OAMD object reconstruction + stereo speaker rendering** (ETSI TS 103 420) — contributed by @ssupt in [#15](https://github.com/OxideAV/oxideav-ac3/pull/15). The E-AC-3 audio-block `skipfld` bytes are now captured at the existing bit cursor (previously discarded) and, when the `addbsi` EC-3 Extension Type A signal is present, parsed as an EMDF container carrying the OAMD and JOC payloads: bounded dense/sparse JOC matrix Huffman decode, up to 16 object signals reconstructed through the normative 64-band complex QMF pipeline, dynamic OAMD object activity/gain/position/size/ramp updates, and an equal-power stereo **speaker** rendering policy (documented as standards-derived — not a claim of bit-parity with any licensed renderer). Exposed as the explicit opt-in factory `decoder::make_eac3_decoder_with_joc`; every existing factory keeps its historical §7.8 compatibility-downmix behaviour, and the opt-in decoder falls back to that same path on absent, malformed, incomplete, or unsupported JOC metadata (invalid optional metadata can never fail an otherwise-decodable stream). Supports the standardized five-channel JOC base presentation (`downmix_config` 0/3) with dynamic-object OAMD assignments; decode/presentation only — no JOC encoding.

- fuzz: fifth coverage-guided target `joc_oamd_parse` riding the #15 JOC contribution — Extension Type A signalling, the EMDF container walk (payload-config validation + sparse/dense Huffman matrix decode), the standalone OAMD object-element decoder on a persistent reuse/differential state, and the QMF reconstruction + stereo renderer on every container that parses (PCM seeded from the input tail; corpus seeded with synthetic valid EMDF containers). The JOC/OAMD/EMDF entry points and renderer are exposed `#[doc(hidden)] pub` per the crate's tests/fuzz convention — none of it is stable API; the documented factory `make_eac3_decoder_with_joc` is the supported surface.

- docs r454: README status rewrite — fractional-syncframe + TPNP encoder sections, a Fuzzing section (four coverage-guided targets + the daily workflow), and a measured **black-box encoder-position ladder** on a shared two-tone fixture through the external decoder binary: the reference encoder is near-transparent (69–88 dB); our AHT path scales with rate (46.7 → 74.9 dB across mono/stereo 96–384 kbps) while the standard path sits flat at ~24 dB — its SNR-offset tuner saturates the csnroffst ceiling on sparse spectra and pads the surplus (recorded rate-distortion follow-up). Stale “SPX and AHT are out of scope on the encoder side” module docs corrected.

- eac3 r454: **transient-pre-noise-processing emission** (§3.7 / §2.3.2.21-23 — the last E-AC-3 encoder-side syntax family) — `eac3::make_encoder_with_tpnp` / the `tpnp` option: per frame and per fbw channel the encoder runs the §3.7.1 transient-location analysis (4-sample group-energy onset detector with running-average reference) and emits `transproce` + `chintransproc[ch]` + `transprocloc[ch]` (frame-relative, 4-sample units) + `transproclen[ch]` (the pre-noise gap between the containing block's leading edge and the transient — the region the decoder's §3.7.2 synthesis overwrites; the exact length analysis is encoder tuning, sketched only by Figure E3.2). Long transforms are forced while TPNP is on (it is the pre-noise tool for long-transform coding); transient-free frames emit `transproce = 0` and are byte-identical to the baseline encoder. The SNR tuner reserves the TPNP words. Validated: audfrm parse-back pins loc (±4 groups) and len exactly on 6-block AND 2-block fractional frames, stationary-content byte-parity with the baseline, options-vs-typed byte-identical, end-to-end decode through our own §3.7.2 synthesis (exact sample counts, decode within a 15 dB gross-desync floor of the baseline decode), a TPNP corruption sweep in `tests/robustness.rs`, and the external decoder binary accepts the syntax (27.4 dB on impulse-train content through the black-box harness).

- fuzz r454: **coverage-guided fuzz standup** (`fuzz/` + the daily `Fuzz` workflow, fleet `crate-fuzz.yml` convention): four libfuzzer targets — `parse_headers` (syncinfo + base-AC-3 BSI + Annex E BSI incl. the metadata opt-in chains and the fractional-frame convsync tail), `decode_frames` (stateful registry AC-3/E-AC-3 packet→PCM decode over carved multi-packet feeds), `eac3_substream_walk` (persistent `Eac3DecoderState` over syncinfo-split frames — substream accumulation + §E.3.8.2 chanmap combination), and `encode_decode_roundtrip` (structure-aware: fuzz-picked contract-valid config across layout × rate × blocks × SPX/AHT/ecpl × metadata; every emitted packet must decode through our own decoder with the exact sample count). Corpora seeded from the docs fixture set. The round-trip target immediately paid for itself (see the bit-budget entry above).

- eac3 r454: three encoder bit-budget findings from the new structure-aware encode→decode fuzz target, each previously a first-frame `bit budget overflow` error on a construction-accepted config: (1) the minimum-rate floor ignored the metadata reserve (1-block mono 96 kbps + dynrng/dialnorm overflowed) — the floor now folds in the actual `Eac3Metadata::reserve_bits` and re-runs in every constructor; (2) the SNR-offset tuners' no-fit paths returned the caller's DEFAULT offsets (csnroffst = 15 — guaranteed overflow) — they now degrade to the floor allocation (csnroffst = 0, all fine offsets 0 ≈ universal bap 0), and the D45 anchor demotion extends to 6-block frames with an ecpl-aware overhead probe; (3) fixed tool syntax that cannot fit its frame at all (7.1 @ 96 kbps total with SPX+ecpl) is now rejected at construction by a **worst-case dry-run**: every constructor encodes one syncframe of deterministic full-scale noise through the real emission pipeline and resets the state, so unfittable configurations fail at `make_encoder*` time instead of on the caller's first frame.

- eac3 r454: **fractional syncframes on the encoder side** (§E.2.3.1.5 `numblkscod` 0/1/2, Table E2.4) — `eac3::make_encoder_with_blocks` / the registry `blocks` option (`1`/`2`/`3`/`6`): a syncframe carries 1, 2, or 3 audio blocks with the byte budget scaled by nblks/6 (unchanged bit rate), the §E.2.3.1.64 `convsync` flag marking each 6-block AC-3 conversion-group head, Table E1.3's implicit `expstre = 1` / `ahte = 0` + explicit `convexpstre` audfrm arms, and `blkstrtinfoe` absent on 1-block frames. Fractional shapes ride the standard coding path (AHT is spec-implicit-off; SPX / enhanced coupling stay 6-block scope) across every layout incl. the 7.1 indep+dep pair. Two guards ride along: a construction-time **minimum-rate floor** (the fixed syntax + D45 exponent payload must fit the shrunken frame — 1-block stereo @ 96 kbps is rejected up front) and a **budget-aware D45 demotion** (the strategy picker's smoothness-preferred D15 anchor could alone exceed a 1-block frame; the plan demotes before exponent quantisation instead of overflowing at pack). Round-trips: per-frame BSI syntax walk (numblkscod / frmsiz / convsync cadence) for 1/2/3-block mono/stereo/5.1/7.1-pair streams, decode-alignment floors vs the 6-block encode of the same PCM (measured ~24-28 dB for 3-block, ~24 dB 2-block, ~13 dB for the 1-block extreme — the honest overhead-amortisation cost), options-vs-typed byte-identical, flush padding to the fractional frame size. Black-box: the external decoder binary accepts all three shapes (stereo 3/2-block @ 192 kbps at 23.4 / 26.0 dB vs the 22.3 dB 6-block baseline harness figure; 1-block @ 384 kbps at 62.0 dB).

- eac3 r454: **decode-conformance fix — `convsync` / `blkid` / `frmsizecod` sit OUTSIDE `if (infomdate)`** (Table E1.2): the Annex E BSI parser read the §E.2.3.1.64-65 tail inside the informational-metadata block, so any fractional-frame stream *without* an informational block desynced the BSI tail by 1 bit (and an AC-3-convert stream by up to 7). The tail is now parsed unconditionally in the main BSI walk, `frmsizecod` is correctly gated on `blkid` (previously read even when `blkid = 0`), and `convsync` is surfaced as a typed accessor (`Eac3Bsi::convsync: Option<bool>`) — the encoder round-trips pin the cadence per frame.

- tests r454: the shared `ffmpeg_decode` harness keyed its temp files on the process id alone, so concurrently running interop tests in one test binary could delete each other's input mid-decode (surfaced as a spurious "ffmpeg rejected the stream"); file names now carry a per-call counter.
...
- eac3 r409: **encoder-side E-AC-3 metadata surface** (`eac3::Eac3Metadata` / `make_encoder_with_metadata` + the same-named registry options) — fixed-BSI `dialnorm` + `compr`, the per-block `dynrng` word (every block of every substream), the Table E1.2 **mixing-metadata block** (`dmixmod`, LtRt/LoRo centre+surround mix levels, `lfemixlevcod`, `pgmscl`, `extpgmscl`; acmod-gated arms per the syntax) and the §E.2.3.1.62+ **informational block** (`bsmod`, copyright/original, 2/0 `dsurmod`+`dheadphonmod`, ≥6-ch `dsurexmod`, audio-production info incl. `adconvtyp`, `sourcefscod`). Blocks ride the independent substream; a 7.1 pair's dependent substream keeps `mixmdate = infomdate = 0` while sharing dialnorm/compr. The SNR tuner budget is reduced by the exact extra bits. Decoder-side: the Annex E BSI parser now surfaces `bsmod` (`Bsi::bsmod: Option<u8>`, previously parsed-and-discarded). Round-trips: every word reads back through the typed Annex E BSI surface per syncframe (5.1 + 2/0 + 7.1-pair), a −12.04 dB dynrng word scales the E-AC-3 decode by exactly 0.25, options-vs-typed byte-identical, reserved codepoints rejected.

- ac3 r409: black-box gain-word semantics gates — the external decoder binary reproduces our authored DRC words exactly: `dynrng` 0xC0 decodes at 0.2500 vs DRC-off (authored 0.2500, Δ0.000 dB), `compr` 0xDF under heavy compression at 0.2422 (authored 0.2422, Δ0.000 dB), and a 10 dB `dialnorm` delta (21 vs 31) measures −10.03 dB under target-level normalisation (±0.5 dB gates; skipped gracefully when the binary is unavailable).

- ac3 r409: **encoder-side bitstream-metadata surface** (`encoder::MetadataParams` / `make_encoder_with_metadata` + registry options `dialnorm`, `compr`, `dynrng`, `bsmod`, `cmixlev`, `surmixlev`, `dsurmod`, `langcod`, `mixlevel`+`roomtyp`, `copyright`, `origbs`) — the §5.4.2 BSI advisory words (dialogue normalisation, Table 7.30 heavy-compression word, bit-stream mode, Table 5.9/5.10 downmix mix-level codes, Dolby Surround flag, deprecated language-code slot, §5.4.2.13-15 audio-production info, copyright/original bits) plus the §5.4.3.3-4 per-block `dynrng` dynamic-range word (emitted in every block; the SNR-offset tuners' byte budget shrinks by the extra words so the mantissa payload still fits). Round-trips: every word reads back through the typed `bsi::parse` surface on every syncframe (5.1 for cmixlev/surmixlev, 2/0 for dsurmod), a −12.04 dB `dynrng` word scales our own decoder's §7.7.1 line-out output by exactly 0.25 (±0.5 dB gate), options-vs-typed byte-identical, out-of-range codepoints rejected at construction.

- eac3 r409: §E.3.5.5.1 factor-2 overlap-add erratum reconciled with the codified clean-room errata (`docs/audio/ac3/ac3-errata.md` entry E3, which also records the ETSI TS 102 366 V1.4.1 clause-absence caveat — the operative text is A/52:2018). The r406 correction is confirmed unchanged; two new erratum-anchored regression pins land both directions: `carrier_overlap_add_factor2_erratum_identity` (decode-side least-squares identity fit — corrected chain gain 1.0, a local as-printed re-implementation measures exactly 0.5, and the two differ by exactly the step-3 multiplier) and `ecpl_roundtrip_unity_level_pins_factor2_erratum` (full encode→decode bitstream round-trip gating the aggregate enhanced-coupling-region energy within ±1.5 dB of unity, vs the as-printed −6.02 dB). The ecpl real-stream fixture GAP remains recorded in `docs/audio/ac3/fixtures/eac3-ecpl-enhanced-coupling/GAP.md`.

- eac3 r406: third enhanced-coupling decode-conformance fix — the resolved banding structure now masks entries up to and including `max(ecpl_begin_subbnd, 8)` per §E.2.3.3.19 (the Table E2.14 default carries a merge bit at sub-band 9; a default-banded region beginning there made `necplbnd` disagree with the §E.3.5.5.1 band walk by one band — a coordinate-count desync). Applied in `parse_strategy` and the encoder geometry; plus narrow-geometry (begin sub-band 9, Table E3.8 middle arm) and 44.1 kHz enhanced-coupling round-trips.

- eac3 r406: **coherence-driven chaos coordinates** (§E.2.3.3.25 / Table E3.12 / §E.3.5.5.3) — the last enhanced-coupling coordinate dimension: the encoder measures per-band coherence `|Σ X·conj(Z)|/sqrt(E_X·E_Z)` against the quantised carrier, maps the incoherent fraction onto the 8-step chaos grid, and pre-divides the transmitted amplitude by the decoder's `1 + 0.38·chaosval` modification (backing chaos off when the pre-compensated amp would exceed the Table E3.10 ceiling). Default on (`EcplParams::chaos` / option `ecpl_chaos`); a partial-coherence stereo-width round-trip gates the effect (decoded in-region inter-channel coherence 0.914 chaos-less → 0.796 with chaos, orig 0.706, band energies still matched) alongside quantiser/coherence unit tests.

- eac3 r406: **SPX + enhanced coupling co-active** (§3.6.1 mid-range coupling + high-range SPX) — `make_encoder_with_spx_ecpl` / options `spx=1`+`ecpl=1`: the coupling region is SPX-bounded (`ecplendf` not transmitted per §E.2.3.3.17, end sub-band derived from `spxbegf` so the regions abut exactly), every fbw channel participates in both tools (mixed `chinspx` rejected in this configuration). Stereo three-region round-trip gates coupling-band / SPX-band / low-band energies; registry-vs-typed pinned byte-identical.

- eac3 r406: ecpl hardening + instrumentation — enhanced-coupling stream through the `tests/robustness.rs` truncation / bit-flip / garbage families; 7.1 indep(coupled)+dep(plain) pair round-trip; `eac3_rate_curves` gains the ecpl column; README documents the subsystem + the external-validator gap (its decoder reports enhanced coupling unimplemented, so ecpl validation is in-tree round-trip + spec-text only)

- eac3 r406: **encoder-side enhanced coupling** (`eac3::make_encoder_with_ecpl` / the `ecpl`, `ecpl_begf`, `ecpl_endf` options) — the last Annex E encoder tool gap. Every fbw channel of the independent substream is coupled: one shared carrier channel (the first coupled channel's MDCT, per-band scaled ~3 dB above the loudest channel) is coded through the standard exponent/bap/mantissa path over the §E.2.3.3.16-17 region, and per-band Table E3.10 amplitude + Table E3.11 angle coordinates are measured in the §E.3.5.5.1 complex analysis domain against the **quantised** carrier (decoder-faithful, incl. the cross-frame carried block and the zero streaming edge), refreshed on blocks 0/3 with §2.3.3.21-22 reuse thrift. Round-trips: stereo band-energy (±3 dB per signal band), stereo quadrature-phase (angle path pinned by an 18 dB waveform floor), 5.1 explicit-chincpl (pins the decoder field-order fix), registry-vs-typed byte-identical, option validation/mutual exclusions (aht/spx).
...
- ac3/eac3 round 373 (r373): **two decoder panics on adversarial / malformed input** (denial-of-service safety). A media decoder fed untrusted bytes must never abort the process. Two slice-index panics were reachable from hand-corrupted frames: (1) **`eac3::ecpl` inverted enhanced-coupling sub-band range** — `ecplbegf`/`ecplendf` are raw 4-bit codes, so a malformed frame can decode to `ecpl_begin_subbnd >= ecpl_end_subbnd` (e.g. `ecplbegf == 15` → begin 20, paired with a low `ecplendf`); the resulting inverted `begin..end` range panicked `necplbnd` / `band_bin_counts` ("slice index starts at N but ends at M"). Now rejected at parse time in `parse_strategy` with `Error::invalid` (mirroring the §E.2.3.3.4-6 SPX sub-band-range guard), and both slice helpers additionally clamp their bounds defensively (§E.2.3.3.16-17). (2) **`audblk::dsp_block` coupling decouple** — the §7.4.2 coupling region spans at most 18 sub-bands (bins 37..253), so `cpl_bndstrc` / `cpl_coord` / `cpl_phsflg` / the `sbnd2bnd` lookup are all sized 18; a malformed `cpl_nsubbnd > 18` (reachable when an E-AC-3 enhanced-coupling span of up to 22 sub-bands threads into the standard decouple path) indexed `sbnd2bnd[18]` out of bounds. The decouple walk and the E-AC-3 `cpl_nbnd` merge loop now clamp to the 18-sub-band maximum, degrading to the valid prefix. New `tests/robustness.rs` drives all 18 corpus fixtures through truncation / multi-bit-flip / sync-prefixed-garbage corruption (~300k feeds locally; the committed test runs a deterministic subset) plus a targeted `eac3_malformed_coupling_range_does_not_panic` regression, asserting the process survives every feed. No change to any valid-stream decode (all PSNR gates unchanged).
- ac3: **railed ±32767/±32768 sample bursts on multi-tone / real content** (issue #10). The §7.2.2.3 log-addition table (`LATAB`, ATSC A/52:2018 Table 7.14) had eight wrong entries across four mid-table runs — `latab[125]` (A=12,B=5), `latab[132..=133]` (A=13,B=2..3), `latab[142..=144]` (A=14,B=2..4), and `latab[176..=177]` (A=17,B=6..7) — each one too low because the preceding run was transcribed one element too short. Understated `latab[]` understated the §7.2.2.3 `bndpsd` PSD integration, which understated the masking curve and starved the high-frequency bins of bits, so their mantissas overflowed to full scale. A pure single tone never reaches the corrupted addresses, so it stayed clean (matching the bug report). `LATAB` is now transcribed fresh, one A-row per line, directly from spec Table 7.14; a new `tables::tests::latab_matches_spec_table_7_14` audit test pins all 256 entries against an independently-transcribed oracle, and an ffmpeg-gated `tests/multitone_railing.rs` integration test confirms a four-tone 384 kbps stream (which railed ~9% of samples pre-fix) now decodes with zero full-scale samples, with a single-tone control. Fixes #10.

### Other

- ac3 round 370 (r370): **gated the two low-rate E-AC-3 corpus fixtures at loose `MinPsnr` floors** (`tests/docs_corpus.rs`). `eac3-low-bitrate-32kbps` (~66.3 dB min-channel) and `eac3-low-rate-stereo-64kbps` (~71.7 dB, high-coupling 64 kbps stereo) decode deterministically; their residual is the lossy floor at those budgets, concentrated in the signal attack/release blocks (the steady interior decodes at ~85 dB+). Promoted from `ReportOnly` to loose 60 dB / 65 dB floors respectively — well below the achieved PSNR — so a gross decode regression fails CI without pinning the lossy-edge headroom. After this round only `ac3-low-bitrate-32kbps-mono` remains `ReportOnly`.
- ac3 round 370 (r370): **promoted 10 base-AC-3 corpus fixtures from `ReportOnly` to CI-gated `MinPsnr` floors** (`tests/docs_corpus.rs`). The base AC-3 decode is bit-stable (identical PSNR across repeated runs) and every 48 / 44.1 / 32 kHz fixture except the torture-grade 32 kbps mono case decodes at 86-92 dB min-channel PSNR — yet all of them were `ReportOnly`, so a regression in the §7.1 exponent / §7.2 bit-allocation / §7.3 mantissa / §7.9 IMDCT base path would not have failed CI. Gated `ac3-{2-1-256k, 3-0, 3-2-384k, 3-2-lfe-448k, stereo-192k, with-audprodie, with-dialnorm-set}`, plus the `ac3-44100hz-stereo` and `ac3-32000hz-stereo` rate variants (the latter confirming the earlier "1.34× RMS inflation" 32 kHz bit-allocation defect is fully resolved — now ~92 dB, max_abs_err=2) at an 80 dB floor, and `ac3-mono-96k` at 78 dB. `ac3-low-bitrate-32kbps-mono` stays `ReportOnly` (its ~62 dB reflects the lossy floor at 32 kbps, with the residual confined to the tone attack/release blocks). No decoder change.
- ac3 round 370 (r370): **re-armed the suspended short-block coverage test** (`tests/ffmpeg_fixture.rs::transient_fixture_has_short_blocks`). The test had been `#[ignore]`d since round 12 because the black-box-validator-encoded `transient_bursts_stereo.ac3` fixture turned out to contain *all long blocks* (the external encoder's psychoacoustic block-switch decision never crossed its threshold on that signal), so the assertion that the fixture carries short blocks could never hold. Rewritten to generate the transient stream **in-process** with the in-tree encoder: a per-block 6 kHz-carrier Gaussian click train reliably drives the §8.2.2 transient detector to flip `blksw=1` on 374/378 blocks. The test now gates the *whole* short-block path end-to-end — §8.2.2 detect → §5.4.3.1 emit → §5.4.3 parse → §7.9.4.2 256-point IMDCT round-trip — and asserts ≥ 30 short blocks plus a non-silent finite self-decode (RMS > 50). No external-encoder dependency; deterministic.
- ac3 round 370 (r370): **re-armed the suspended 5.1 coupling test** (`encoder::tests::five_one_coupling_beats_no_coupling_at_low_bitrate`). The test asserted a ≥ 1 dB self-decode PSNR *win* for §7.4 coupling-on over coupling-off at a constrained 320 kbps 5.1 budget; that premise does not hold for the in-tree encoder (both paths land on the same ~12 dB floor, matching the 10 dB floor the rest of the 5.1 suite gates on), and it had been failing whenever run under `--ignored`. The `build_multichan_hf_pcm` fixture was also pathological for coupling — distinct per-channel HF tones are the *worst* case, since the coupling channel carries one shared coefficient set recovered per-channel only through an amplitude coordinate. Rebuilt the fixture HF-*correlated* (the same 8/11/14 kHz complex in every fbw channel, level-panned, channel identity in a distinct sub-boundary LF tone) and re-pointed the assertions at deterministic invariants: coupling-on engages (`cplinu=1` on 48/48 blocks), coupling-off suppresses it (0/48), bitstream sizes match at equal `frmsizecod`, and the coupling-on decode does not regress (≥ 10 dB floor, within 0.5 dB of coupling-off). The test stays `#[ignore]`d only for its legitimate `AC3_DISABLE_CPL` env-var race isolation; it now passes when run with `--ignored`.

- eac3 round 365 (r365): **per-block SNR-offset strategies `snroffststr ∈ {1, 2}`** (§2.3.2.3 / §2.3.3.27 / Table E1.3-E1.4). The E-AC-3 audblk DSP (`eac3::dsp::decode_indep_audblks`) now decodes the two per-block SNR-offset packing modes the spec defines alongside the previously-handled frame-level `snroffststr == 0`: mode `0x1` carries a single shared `blkfsnroffst` (applied to coupling + every fbw channel + LFE), and mode `0x2` carries an independent fine offset per coupling/channel/LFE slot. Both emit `csnroffst` (6 bits) per block, gated by `snroffste` (implicit 1 in block 0, explicit thereafter, with prior-block reuse when 0). This removes the `snroffststr != 0` whole-frame reject in `reject_unsupported` (which fell back to silent PCM). The in-tree encoder gains a test-only `make_encoder_with_snroffststr` path that emits both modes (reserving the extra per-block header bits out of the mantissa budget so the syncframe still fits); the default public `make_encoder` is byte-identical (still `snroffststr == 0`). A new self-encode → self-decode round-trip test (`snroffststr_per_block_roundtrip_{mono,stereo,51}`) drives both modes for 1.0 / 2.0 / 5.1 layouts and asserts the `0x1` and `0x2` decodes are bit-identical to each other (the strict cursor-alignment invariant — a one-bit misparse would corrupt every downstream mantissa) plus a ≥ 55 dB floor versus the frame-level baseline. No corpus fixture exercises `snroffststr != 0` (every validator-encoded fixture picks `0`), so this is validated by round-trip rather than against the corpus. 3 new lib tests (367 → 370).
- eac3 round 365 (r365): promoted the `eac3-5.1-side-768kbps` corpus fixture from `ReportOnly` to a CI-gated `MinPsnr(80.0)` floor — it decodes stably at ~91.7 dB on every channel, matching the other known-good E-AC-3 multichannel fixtures, so a regression in the 5.1 decode path is now caught.

- ac3/eac3 round 355 (r355): **dynamic-range-control + dialogue-normalisation control surface** (§6.1.9 / §7.6 / §7.7) — new `drc` module (`DrcSettings` / `DrcMode`, re-exported at the crate root) implementing the listener-controllable gain layer the spec wraps around the mandatory §7.7.1 default decode. (1) §7.7.1.2 **partial compression**: `scale_dynrng_partial` reinterprets the `dynrng` word as a signed 8-bit fraction and applies independent cut (gain-reduction) and boost (gain-increase) factors; factor 1.0 reproduces full compression bit-for-bit over all 256 codes, 0.0 flattens to the 0 dB code (original dynamic range). (2) §7.7.2.2 **heavy compression** ("RF mode"): `compr_to_linear` decodes the 4+4-bit `compr` word (±48 dB, Table 7.30) and substitutes it for `dynrng`, falling back to `dynrng` when a frame carries no `compr` word (§7.7.2.1). (3) §7.6 **dialogue normalisation**: `dialnorm_gain` derives the opt-in playback scalar `10^((target − dialnorm)/20)` toward a chosen headroom target. Wired through **both** decode paths — `Ac3State` gains a `drc` field; the AC-3 audblk (§5.4.3.3-6) and E-AC-3 dsp (§E.1.3.2) `dynrng`/`dynrng2` decodes route through `resolve_block_gain`; the dialnorm scalar is applied pre-downmix (AC-3 f32, E-AC-3 passthrough-S16 + downmix-f32). Public API: `decoder::make_decoder_with_drc(params, DrcSettings)`. The two duplicated local `dynrng_to_linear` helpers (audblk + eac3::dsp) collapse into the shared `drc` copy. Default is line-out, so the mandatory §7.7.1 decode — and every PSNR-gated corpus fixture — is byte-unchanged. 18 new tests (13 unit gain-math: Table 7.29/7.30 endpoints, partial-comp identity/zero/halving/direction-independence, dialnorm symmetry + reserved codepoints; 5 end-to-end decoder: dialnorm target scales the sine440 fixture RMS by the closed-form gain within 2%, partial(0,0) + RF-without-compr no-ops, reset preserves the setting).
- ac3 round 348 (r348): **short-block (block-switching) transform now spec-correct per §7.9.4.2 / §8.2.3.2** — the two 256-sample sub-transforms of a `blksw[ch] == 1` block use *different* phase parameters (`α = −1` for the first short transform, `α = +1` for the second), and both the decoder IMDCT (`imdct::imdct_256_pair_fft`) and the encoder MDCT (`mdct::mdct_256_half`) must realise that distinction. The previous code applied the X1 (`α = −1`) §7.9.4.2 step-5 de-interleave pattern `(−yi, yr, −yr, yi)` to *both* halves; the X2 half must instead use the verbatim spec pattern `(−yr, yi, yi, −yr)`. Verified numerically: the corrected FFT path reproduces the α-parameterised direct-form 256-point IMDCT of each sub-block to f32 precision (residual ≈ 0), whereas the old X2 pattern was structurally decorrelated from it. The forward `mdct_256_half` gains the matching `alpha` parameter so analysis-synthesis still round-trips (encoder transient self-decode tests stay green). Test `imdct::tests::imdct_256_pair_fft_matches_alpha_direct_form` replaces the previous TDAC-symmetry gate (which encoded the incorrect "both halves antisymmetric" assumption). NOTE: the `ac3-32000hz-stereo` corpus fixture stays ReportOnly at ~13 dB — its base-AC-3 defect is a *separate* block-0 mantissa bit-accounting drift that mis-reads block 1 as block-switched; root-caused to a 4-bit difference between our decoder's §7.2.2 bit-allocation result (1280 mantissa bits for block 0) and the reference encoder's (1284). Our bit-allocation `bap[]` was verified line-for-line against the A/52 §7.2.2 pseudocode and an independent from-scratch reimplementation (both agree exactly at 1280), so the residual is a spec-vs-reference-encoder discrepancy in the masking/`bap` derivation for this specific 32 kHz / D15 content, not a transcription bug — see r348 report.
- ac3 round 348 (r348): reset `deltnseg[]` (delta-bit-allocation segment counts, incl. the coupling pseudo-channel) to 0 at the start of every syncframe, per the §7.2.2.6 / §5.4.3.47 initialisation note ("initialize the cpldeltnseg and deltnseg[ch] delta bit allocation variables to 0 at the beginning of each syncframe"). Prevents a `deltbaie == 0` block 0 from inheriting a previous frame's dba segments and applying a phantom masking-curve offset.
- ac3/eac3 round 345 (r345): corpus driver now derives the true elementary-stream sample rate from the first syncframe header (`tests/docs_corpus.rs::stream_sample_rate` — AC-3 syncinfo Table 5.6 `fscod`, E-AC-3 `(fscod, fscod2)` via `eac3::bsi::parse_with`) instead of hard-coding 48 kHz. This was mis-labelling every non-48 kHz fixture; `ac3-32000hz-stereo` no longer reports a 48 kHz/32 kHz mismatch. (The 32 kHz decode itself still carries a systematic ~1.34× RMS inflation in base bit allocation — tracked as a follow-up, separate from the sample-rate label.)
- eac3 round 345 (r345): **standard-coupling default banding structure** (§E.2.3.3.15 Table E2.12). When `cplbndstrce == 0` in the first coupling block of an E-AC-3 frame, the spec mandates the default coupling banding structure `defcplbndstrc[]` (indexed by absolute coupling sub-band number) rather than an all-zeros / fully-un-merged structure. The `eac3::dsp` audblk coupling parse previously cleared every `cpl_bndstrc[bnd]` to `false` on the `cplbndstrce == 0` path, so a 7-sub-band coupling region that should collapse to 3 bands stayed at 7 — corrupting the per-band coupling-coordinate scatter (§7.4 decouple) and wrecking every basic stereo-coupled frame. Added the `DEFCPLBNDSTRC` Table E2.12 constant + a first-coupling-block tracker; a later-block `cplbndstrce == 0` correctly reuses the prior structure. This lifted the three previously floor-bound corpus fixtures `eac3-stereo-48000-192kbps`, `eac3-256-coeff-block`, and `eac3-from-ac3-bitstream-recombination` from 8.36 / 8.36 / 13.57 dB to 90.88 / 90.88 / 91.04 dB PSNR. `tests/docs_corpus.rs` gains a `Tier::MinPsnr(f64)` tier and promotes the three fixtures (plus the known-good `eac3-5.1-48000-384kbps`) to CI-gated 80 dB floors.
- eac3 round 321 (r321): §E.3.3.2 `nrematbd` now folds in enhanced coupling. The 2/0 rematrix-flag field count (eac3::dsp audblk loop) previously routed an `ecplinu == 1` block through the standard-coupling arm keyed on `cplbegf` (which the enhanced-coupling path never sets), drifting the bit cursor on enhanced-coupling 2/0 frames. `audblk::remat_band_count_spx` gains an `ecpl_in_use` / `ecplbegf` pair and implements the §E.3.3.2 enhanced-coupling arm verbatim (raw `ecplbegf` 0/1/2/<5 → 0/1/2/3 bands, else 4 — uniquely admitting `nrematbd = 0`). The raw `ecplbegf` code is now retained on `eac3::ecpl::EcplStrategy` (carried across `cplstre == 0` strategy-reuse blocks). New unit test `audblk::spx_tests::nrematbd_e3_3_2_decision_tree` covers every arm. Resolves one of the two §E.3.5.5 enhanced-coupling follow-ups (cross-frame edge threading remains).
- eac3 round 317 (r317): enhanced-coupling (`ecplinu == 1`) decoder-level integration — §E.3.5.5 now decodes end-to-end. The audblk parser reads the §E.2.3.3.16-26 strategy + per-channel amplitude/angle/chaos coordinates (eac3::ecpl::parse_strategy / parse_coords) and decodes the enhanced-coupling channel through the shared exponent / bit-allocation / mantissa path by pinning the coupling region at the ecpl bins; a deferred second pass (eac3::dsp::run_deferred_ecpl_dsp) reconstructs the §E.3.5.5.1 carrier `Z[k]` from prev/curr/next blocks and emits each coupled channel's transform coefficients via eac3::ecpl::synthesize_block (carrier × per-bin amplitude × de-correlated angle, §E.3.5.5.4), replacing the standard §7.4 decouple (gated by Ac3State::skip_decouple). Cross-block random de-correlation sources held on Ac3State::ecpl_state (EcplState). Removes the audblk-stage `ecplinu == 1` `Unsupported` rejection. Frame-edge neighbours use zero carriers (§E.3.5.5.1 "set to zero"); cross-frame edge threading + the 2/0 ecpl+rematrix band-count interaction are follow-ups.
